...
//               队列左边< i-size 的部分是过期的窗口，需要从队列中删除
                while (!q.isEmpty() && q.peekFirst() <= i - size)
                    q.pollFirst();//pullFirst取出的是队列左边的元素

//               从队列的右边压入元素， 压入的元素如果大于当前窗口右边的元素，则可以将窗口右边较小的元素删除（此时需要保证这个窗口的元素从左到右是单调递减的）
//               注意只有从窗口的右边进行比较删除操作才能使得窗口保持单调递减
                while (!q.isEmpty() && num[q.peekLast()] <= num[i])
                    q.pollLast();//pollLast取出的是队列右边，比新到的元素要小的元素
//                将新到的元素从队列右边压入
                q.offerLast(i);
//                当窗口的长度到达size长时开始输出元素。
                if (i >= size - 1)
                    res.add(num[q.peekFirst()]);//一直取队列的左边元素，因为队列从左往右是单调递减的，最左边就是最大的元素
            }
        }
        return res;
    }
